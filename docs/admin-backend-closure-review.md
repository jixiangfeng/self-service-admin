...
平台基础配置与系统权限 -> 商户建档 -> 门店/点位/设备建档 -> 服务商品与计费配置 -> 订单交易 -> 履约核销 -> 用户资产沉淀 -> 营销活动 -> 结算/分润/发票 -> 客服消息与评价反馈 -> 数据分析与运营支撑。
...
| 运营支撑 | 运营支撑、支付运维、设备履约运维、运营配置、扩展、文件、审批、风控调度、小程序、开放接口、平台基础 | `/payment-orders`、`/payment-channels`、`/payment-reconciliations`、`/device-commands`、`/device-faults`、`/file-assets`、`/approval-tasks`、`/risk-rules`、`/scheduled-jobs`、`/mini-program-page-configs`、`/open-api-clients`、`/system-configs` 等 | 平台配置、支付和设备运维、文件留存、审批流、风控、调度、小程序和开放 API | 保障经营链路稳定运行 |
