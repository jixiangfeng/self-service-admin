...
const AssetManagement: React.FC = () => {
  ...
  const rechargeColumns: ProColumns<RechargeOrderRecord>[] = [
    ...
    { title: '可用范围', dataIndex: 'scopeType', width: 120, search: false, render: (_, record) => formatEnumText(record.scopeType, 'scopeType', '可用范围') },
    { title: '结算规则', dataIndex: 'settlementRule', width: 160, search: false, renderText: (value, record) => [formatEnumText(record.settlementMode, 'settlementMode', '结算模式'), value].filter(Boolean).join(' / ') || '-' },
    { title: '结算状态', dataIndex: 'settlementStatus', width: 120, search: false, render: (_, record) => formatEnumText(record.settlementStatus, 'settlementStatus', '结算状态') },
