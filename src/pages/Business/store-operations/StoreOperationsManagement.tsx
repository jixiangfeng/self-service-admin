...
const initialTasks: TaskRecord[] = [
  { id: 'task-1', task: '夜洗门店巡检', owner: '店长-周可', deadline: '今日 18:00', status: 'PROCESSING', store: '徐汇夜洗门店' },
  { id: 'task-2', task: '泡沫设备保养', owner: '运维-李维', deadline: '今日 20:00', status: 'PENDING', store: '虹桥旗舰洗车站' },
  { id: 'task-3', task: '公告更新：五一价格调整', owner: '运营-何铭', deadline: '明日 10:00', status: 'CONFIRMING', store: '嘉定联营门店' },
];

const initialNotices: NoticeRecord[] = [
  { id: 'notice-1', module: '早班排班', content: '店长 1 人，店员 2 人', status: 'PUBLISHED', store: '虹桥旗舰洗车站', publisher: '店长-李思远' },
  { id: 'notice-2', module: '晚班排班', content: '店长 1 人，店员 1 人，运维 1 人待定', status: 'CONFIRMING', store: '徐汇夜洗门店', publisher: '区域运营-何铭' },
  { id: 'notice-3', module: '门店公告', content: '五一期间夜间价格规则调整说明', status: 'PENDING', store: '嘉定联营门店', publisher: '运营-何铭' },
];
...
const StoreOperationsManagement: React.FC = () => {
  ...
  const summary = useMemo(
    () => ({
      ...
      pendingException: tasks.filter((item) => item.status !== 'PUBLISHED').length,
    ...
    <div style={{ padding: 24 }}>
          ...
          <Card title="门店运营任务" extra={<Button type="primary" onClick={() => { taskForm.resetFields(); taskForm.setFieldsValue({ status: 'PENDING' }); setTaskVisible(true); }}>新建任务</Button>}>
            <Table
              ...
              columns={[
                ...
                { title: '状态', dataIndex: 'status', width: 100, render: (value: string) => <Tag color={statusColorMap[value]}>{value}</Tag> },
                {
                  title: '操作',
                  width: 180,
                      ...
                      <Button
                        ...
                        onClick={() => {
                          setTasks((prev) =>
                            prev.map((item) =>
                              item.id === record.id
                                ? { ...item, status: item.status === 'PROCESSING' ? 'CONFIRMING' : 'PROCESSING' }
                                : item
                            )
                          );
      ...
      <Modal title="新建运营任务" open={taskVisible} onOk={handleTaskSubmit} onCancel={() => { setTaskVisible(false); taskForm.resetFields(); }} width={820}>
        <Form form={taskForm} layout="vertical">
          <div className="modal-grid">
            ...
            <Form.Item name="status" label="状态"><Select options={[{ value: 'PENDING', label: '待分派' }, { value: 'PROCESSING', label: '处理中' }, { value: 'CONFIRMING', label: '待确认' }]} /></Form.Item>
          </div>
        </Form>
      </Modal>
      ...
      <Modal title="新建公告 / 排班" open={noticeVisible} onOk={handleNoticeSubmit} onCancel={() => { setNoticeVisible(false); noticeForm.resetFields(); }} width={820}>
        <Form form={noticeForm} layout="vertical">
          <div className="modal-grid">
            <Form.Item name="module" label="模块" rules={[{ required: true, message: '请输入模块' }]}><Input /></Form.Item>
            ...
            <Form.Item name="status" label="状态"><Select options={[{ value: 'PENDING', label: '待发布' }, { value: 'CONFIRMING', label: '待确认' }, { value: 'PUBLISHED', label: '已发布' }]} /></Form.Item>
