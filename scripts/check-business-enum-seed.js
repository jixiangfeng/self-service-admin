...
const dictCodeMatches = [...dmlText.matchAll(/SELECT\s+'[^']+'\s*,\s*'([A-Za-z0-9_]+Options)'\s*,\s*'业务枚举：/g)];
const seededCodes = new Set(dictCodeMatches.map((m) => m[1]));

const itemPattern = /FROM\s+`dictionary`\s+d\s+WHERE\s+d\.\`code\`\s*=\s*'([A-Za-z0-9_]+Options)'[\s\S]{0,280}?AND\s+NOT\s+EXISTS\s*\(SELECT\s+1\s+FROM\s+`dictionary_item`\s+di\s+WHERE\s+di\.\`dict_id\`\s*=\s*d\.\`id\`\s+AND\s+di\.\`value\`\s*=\s*('([^']*)'|\d+)\)/g;
const seededItems = new Map();
for (const match of dmlText.matchAll(itemPattern)) {
  const key = match[1];
  const value = String(match[3] ?? match[2].replace(/'/g, ''));
  if (!seededItems.has(key)) seededItems.set(key, new Set());
  seededItems.get(key).add(value);
}
